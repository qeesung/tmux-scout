...
When `@scout-watchdog` is enabled, tmux-scout keeps hooks as the primary state source and adds reference-style reconciliation: process/pane lifecycle checks, tail-only Codex transcript reads with cached offsets, lower-frequency JSONL discovery, and periodic full reconcile. It does not repeatedly reread every transcript on the fast path.
