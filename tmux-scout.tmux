...
tmux run-shell -b "$restore_path; node \"$CURRENT_DIR/scripts/setup.js\" status --quiet 2>/dev/null || tmux display-message 'tmux-scout: hooks not installed. Run: \"\$(tmux show-env -g SCOUT_DIR | sed s/^SCOUT_DIR=//)/scripts/setup.sh\" install'"
