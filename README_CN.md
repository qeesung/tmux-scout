...
开启 `@scout-watchdog` 后，tmux-scout 仍以 hook 作为主状态源，并增加 参考实现风格的校正机制：进程/pane 生命周期检查、带 offset 缓存的 Codex transcript 尾部增量读取、较低频的 JSONL 发现，以及周期性全量 reconcile。快速路径不会反复全量读取所有 transcript。
